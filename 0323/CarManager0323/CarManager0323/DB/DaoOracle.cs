...
namespace CarManager0323.DB
{
    class DaoOracle
    {
        ...
        public void insertCar(Car car)
        {
            try
            {
                string sql = string.Format("insert into car values(car_SEQ.nextval, "
                    + "'{0}',{1}, '{2}','{3}', '{4}')", car.Model, car.Price, car.Color, car.Year, car.Maker);
                cmd.Connection = conn;
                cmd.CommandText = sql;
                cmd.ExecuteNonQuery();
            }
        ...
        public void insertSeller()
        {
            try
            {
                string sql = "insert into seller values (seller_seq.nextval, " +
                    "'김사원', '010-1111-1234', 'kim@hyndai.com', '대리', '동대구영업소')";
                cmd.Connection = conn;
                cmd.CommandText = sql;
                cmd.ExecuteNonQuery();
            }
        ...
        public void insertCustomer()
        {
            try
            {
                string sql = "insert into customer values (customer_seq.nextval, " +
                    "'김순자', '대구 동구 신천2동', '010-111-1234', 'kimsoonja@naver.com')";
                cmd.Connection = conn;
                cmd.CommandText = sql;
                cmd.ExecuteNonQuery();
            }
        ...
        public void insertDeal()
        {
            try
            {
                string sql = "insert into deal (d_id, c_id, " +
                    "v_id, d_day, s_id) " +
                    "(select deal_seq,nextval, cu.c_id," +
                    "ca.v_id, sysdate, s.s_id " +
        ...
        public void insertDeal2()
        {
            try
            {
                string sql = "insert into deal_t values (" +
                    "deal_t_SEQ.nextval, sysdate, " +
                    "(select s.s_id from seller s natural join car ca where s.s_name='김철수 ')," +
                    "(select distinct ca.v_id from car ca where ca.v_model='그랜저')," +
                    "(select distinct cu.c_id from customer cu where cu.c_name='김순자'))";
        ...
        public void insertDeal(Deal deal)
        {
            try
            {
                string sql = string.Format("insert into deal (d_id, c_id, " +
                    "v_id, d_day, s_id) " +
                    "(select deal_seq,nextval, cu.c_id," +
                    "ca.v_id, sysdate, s.s_id " +
                    "from seller s natural join customer cu" +
        ...
        public void insertDeal(Customer cust, Car car, Seller sell)
        {
            try
            {
                string sql = string.Format("insert into deal values " +
                    "(DEAL_SEQ.nextval, sysdate, " +
                    "(select c_id from customer where c_name = '{0}' and c_tel = '{1}')," +
                    "(select v_id from car where v_model = '{2}' and v_price = {3}), " +
                    "sysdate, " +
                    "(select s_id from seller where s_name = '{4}' and s_tel = '{5}'))",
                    cust.Name, cust.Tel,
                    car.Model, car.Price,
                    sell.Name, sell.Tel);
        ...
        public void insertSeller(Seller sell)
        {
            try
            {
                string sql = string.Format("insert into seller " +
                    "values (seller_seq.nextval, " +
                    "'{0}', '{1}', '{2}', '{3}', '{4}')",
                    sell.Name, sell.Tel, sell.Email, sell.Addr, sell.Office_Name);
                cmd.Connection = conn;
                cmd.CommandText = sql;
                cmd.ExecuteNonQuery();
            }
